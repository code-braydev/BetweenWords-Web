<template>
    <Teleport to="body">
        <div v-if="show"
            ...
            <div v-motion :initial="{ opacity: 0, scale: 0.9 }" :enter="{ opacity: 1, scale: 1 }" :class="[
                ...
                <div :class="[
                    ...
                    <h3 class="text-nebula-primary dark:text-nebula-cyan font-mono text-xs uppercase tracking-widest">{{
                        title }}</h3>
                    <button v-if="!preventClose" @click="$emit('close')"
                        class="text-slate-400 hover:text-red-500 dark:text-white/40 dark:hover:text-red-500 transition-colors">
                        <XIcon class="w-5 h-5" />
                    </button>
                </div>
                <div class="p-6">
