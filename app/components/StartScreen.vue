<template>
    <div class="absolute inset-0 z-50 flex flex-col items-center justify-center bg-[#050505] overflow-hidden transition-all duration-[1500ms] ease-in-out"
        ...
        <div v-motion :initial="{ opacity: 0, y: -80, rotateX: 45 }"
            ...
            class="flex flex-col items-center mb-24 relative z-10 group cursor-default">
            <div class="relative">
                <div
                ...
                <img src="/logo.png" alt="Between Words Logo"
                    class="w-32 h-32 sm:w-44 sm:h-44 mb-6 drop-shadow-[0_0_15px_rgba(245,44,245,0.3)] animate-[bounce_5s_infinite] transform group-hover:scale-105 transition-transform duration-700" />
            </div>
            <h1
                class="text-6xl sm:text-8xl font-black text-transparent bg-clip-text bg-gradient-to-br from-nebula-primary via-white to-nebula-cyan font-mono tracking-tighter uppercase drop-shadow-[0_5px_10px_rgba(0,0,0,0.5)] text-center px-4 hover:scale-105 transition-transform duration-500">
                Between Words
            </h1>
            <div class="flex items-center gap-4 mt-6">
                <div class="h-[1px] w-12 bg-nebula-cyan/20"></div>
                <p
                    class="text-nebula-cyan/60 tracking-[0.5em] uppercase text-[10px] sm:text-xs font-mono text-center font-bold">
        ...
        <button @click="handleStart" v-motion :initial="{ scale: 0, opacity: 0 }"
            ...
            :hovered="{ scale: 1.05 }" :tapped="{ scale: 0.95 }" class="relative group mb-32 z-20 cursor-pointer">
            ...
            <div
                class="relative w-40 h-40 sm:w-48 sm:h-48 bg-black/10 backdrop-blur-xl rounded-full border border-white/5 flex items-center justify-center shadow-xl group-hover:border-nebula-primary/20 transition-all duration-700 overflow-hidden">
                <Play
                    class="w-14 h-14 sm:w-20 sm:h-20 text-white/80 ml-3 group-hover:text-white transition-all duration-700" />
            </div>
        </button>

        <!-- Go to Learning Button -->
        <div v-motion :initial="{ opacity: 0, y: 30 }"
            :enter="{ opacity: 1, y: 0, transition: { duration: 1000, delay: 1000 } }" class="z-10 mt-8">
            <UiButton variant="subtle" size="sm" :icon="GraduationCap" label="REPASAR PRIMERO"
                @click="$emit('learning')"
                class="opacity-40 hover:opacity-100 border border-white/5 hover:border-white/10 transition-all duration-700 px-8 py-4 text-xs tracking-widest uppercase z-20" />
        </div>
    </div>
