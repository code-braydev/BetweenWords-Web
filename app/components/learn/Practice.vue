    ...
    <section v-motion :initial="{ opacity: 0, y: 40 }" :enter="{ opacity: 1, y: 0, transition: { duration: 600, delay: 300, ease: 'easeOut' } }" class="mt-8 p-4 sm:p-6 lg:px-8">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="mb-10 text-center">
                <h2 class="text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tighter mb-2">
                    Learning <span class="text-nebula-primary">Lab</span>
                </h2>
                <p class="text-xs text-slate-500 dark:text-white/40 font-mono uppercase tracking-widest">
                    Selecciona una actividad para comenzar
                </p>
            </div>

            <!-- Tabs Navigation -->
            <div class="flex flex-wrap justify-center gap-2 mb-8 sm:mb-12 bg-slate-100 dark:bg-white/5 p-2 rounded-2xl border border-slate-200 dark:border-white/10 backdrop-blur-md">
                <button v-for="tab in tabs" :key="tab.id"
                    ...
                    <div v-if="activeTab === tab.id" class="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-white rounded-full"></div>
                </button>
            </div>
            ...
            <transition name="page" mode="out-in">
                <div :key="activeTab" class="min-h-[400px] sm:min-h-[500px]">
                    <div class="mb-6 sm:mb-8 flex flex-col sm:flex-row items-center sm:items-start gap-4 p-4 sm:p-6 bg-nebula-primary/5 border border-nebula-primary/20 rounded-2xl text-center sm:text-left">
                        <div class="p-3 bg-nebula-primary/10 rounded-xl">
                            <Info class="w-6 h-6 text-nebula-primary" />
                        </div>
                        <div>
                            <h3 class="text-sm font-black text-nebula-primary uppercase tracking-widest mb-1">{{ currentTabInfo.title }}</h3>
                            <p class="text-[11px] text-slate-600 dark:text-white/60 leading-relaxed">{{ currentTabInfo.description }}</p>
                            <div class="mt-3 flex items-center gap-4">
                                <div class="flex items-center gap-1.5">
                                    <Target class="w-3.5 h-3.5 text-nebula-cyan" />
                                    <span class="text-[10px] font-bold text-slate-500 dark:text-white/40 uppercase tracking-tighter">Objetivo: {{ currentTabInfo.goal }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
